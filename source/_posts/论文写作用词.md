...
**developed** ：is innovatively developed by elaborately creating a self-attention-weighted bidirectional long short-term memory (Bi-LSTM) network in conjunction with 1-dimensional convolution (Conv-1D), named SeaBil.
...
have become a resarch hotspot in the field of ..., because of the advantages of... . However,
...
**conduct**：Recently, shortterm deterministic sea-wave predictions (DSWPs) based on X-band radar measurements have been successfully conducted using phaserevolved wave components reconstructed from radar retrievals.

**show:**Ferrandis et al.18 showed the potential of the data-driven model based on the LSTM model to simulate the nonlinear dynamics of a vessel in real-time.

**propose**:By extension, the encoder–decoder system19,20 was proposed for forecasting ship maneuvering in waves. The records of the various physical variables such as wave elevation, 6 degrees-of-freedom (DOF) motion responses, and rudder angle were input to the encoder, and the future time series of those variables were predicted through the decoder.

**investigate:**Liu et al.22 investigated the prediction performance of the LSTM model according to the length of the adopted motion history (input vector space) in relation to the physical memory effects estimated by the impulse response function or auto-correlation function.

### Introduction对文章结构介绍

**present;introduce;give results and describe;presents the conclusions**:The remainder of this article is organized as follows. Section II **presents** previous research in this domain. Section III **introduces** the predictors and their architecture/parameters. Section IV gives results and describes the vessel and data selected for training and testing of the predictors. Section V presents the conclusions.

The rest of this paper is organized as follows. In Section 2, a deep learning network architecture is **introduced,** and the discussions on the activation function and input feature design are **presented**.In Section 3, the turning test in a real sea area based on JARI-USV, together with data measurement, is **discussed,** and the simulation results based on the prediction of networks with different activation functions and input features are **elucidated**. In Section 4, conclusions on the capability of deep learning networks for the prediction of ship motions are **indicated**, and further perspectives on developing network performance are provided.

Section 2 **introduces** the architecture and theoretical formula of the MSA-LSTM model. The optimization strategy and the two-stage training mechanism is **presented** in section 3. In Section 4, **the results of** ship motion prediction experiment and the analysis of prediction results are **presented.** Section 5 summarizes the results of this research.

The remainder of this article is as follows: The ADWT algorithm and the structure of the hybrid model are **presented** in Section 2. The prediction performance of the hybrid prediction model for ship motion attitude in different ocean conditions is **discussed** in Section 3. Finally, **the conclusion of the experiments and the summary of the entire study** are **represented** in Section 4.

The rest of this paper is structured as follows. In Section 2, a variable-order model of ship maneuvering motion is **proposed** in detail. Section 3 **introduces** the basic algorithms used in the whole program, including the basic principles of EKF, LS-SVM, and BP. Section 4 **describes** the framework of the MDIP model. Section 5 **examines** the simulation data for a container ship and compares in detail the two results **derived** from the MDIP scheme with the optimal-order mathematical model prediction result. Finally, c**onclusions and future research directions are given** in Section 6.

This article is organized as follows. Section II **introduces** graph theory, GVFs, and radial basis function NNs (RBFNNs). Section III **formulates** the containment control problem under switching topologies. Section IV **presents** the proposed cooperative control architecture. Section V **provides** the stability analysis of the closed-loop control system. Section VI **presents** the simulation results. Section VII **concludes** this article.

The rest of this paper is organized as follows: t**he basic principles** of ship roll motion is **described** in Section 2; in Section 3, the overall structure and theoretical formula of the proposed model is presented; Section 4 **analyzes the details of the datasets** and **discusses the experimental results**; the conclusion is summarized in the last section.
...
**A similar study**：A similar study by Silva and Maki21 simulated the 6 DOF ship motions during specific maneuvering scenarios such as turning and course-keeping.
